...
$client = new Web3Client(
    $config['rpc_url'],
    $config['contracts']['MyTokenlogistique']['address'],
    $config['contracts']['MyTokenlogistique']['abi'],
    $config['contracts']['MyTokenlogistique']['private_key']
);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['server_mint'])) {
    try {
        // Appeler la fonction mintcommander
        $txHash = $client->sendTransaction('mintTokenL');
...
<head>
<meta charset="utf-8">
<title>Mint des Tokens logistique</title>
</head>
<body>
<h1>Mint des Tokens logistique</h1>
...
<form method="POST">
<button type="submit" name="server_mint">Mint 1 MyTokenlogistique (via PHP)</button>
</form>
<hr>
<!-- Bouton pour déclencher la fonction JavaScript (signature via MetaMask) -->
<button type="button" onclick="mintWithMetaMask()">Mint 1 MyTokenlogistique (via MetaMask)</button>
...
<script>
    ...
    async function mintWithMetaMask() {
        ...
        // Appeler mintTokenL() côté client, signature par MetaMask
        try {
            const txReceipt = await contract.methods.mintTokenL().send({ from: sender });
